...
The lab also prepares an independently authored memory architecture and runnable apparatus from an empty repository. `research/own-memory-architecture.md` and `research/memory-design-test-map.md` are proposed design/validation studies, not an implemented runtime or accepted system ontology. A future in-house system remains one system under study; its schema must not become the evaluator’s answer format. The existing cross-system promotion gate does not gate ordinary design research or replace the concrete authority needed for a future implementation task. Source studies, articles and the existing Case Lab retain independent value.
