...
| 22 | Public、sealed、operations concerns 需要三个 repos | **REJECTED FOR NOW** | Public/sealed/operations 三分法仍被拒绝。保留的早期实验位于独立 repo；更正后的 ADR-0007 记录该事实，不授权实现计划。未来 Tilia 目标独立于 Relata。 |
...
- R0 不做跨系统 runner、API、SDK、service、Leaderboard、Arena 或 hosted infrastructure；独立 Tilia 目标不授权在 Relata 内实现。
