...
来源连接存在，但它不是对引用内容的真实性担保；`store_raw_episode_content=False` 会把已保存 episode 正文清空，保留 UUID 不等于完整可审计原文。[来源保存条件](https://github.com/getzep/graphiti/blob/16cdf7045378c8d53ae01f94e2fa60d238cb0f68/graphiti_core/graphiti.py#L747-L751) **INFERRED**：应用应明确区分作者、说话人、信息来源及其权威，尤其不能把 agent 自己回写的断言自然升级成人的认可。
